      ...
      <template #header>
        <div class="card-header">
          <div class="header-left">
            ...
            <span class="header-title">电费管理</span>
          </div>
...
import { getElectricityFeeList, createElectricityFee, updateElectricityFee, deleteElectricityFee } from '@/api/electricityFee'
...
const loadData = async () => {
  try {
    ...
    const response = await getElectricityFeeList(params)
...
const handleDelete = async (row) => {
  try {
    ...
    await deleteElectricityFee(row.id)
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        if (isEdit.value) {
          await updateElectricityFee(form.value.id, formData)
          ElMessage.success('更新成功')
        } else {
          await createElectricityFee(formData)

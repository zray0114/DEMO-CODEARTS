      ...
      <template #header>
        <div class="card-header">
          <div class="header-left">
            ...
            <span class="header-title">水费管理</span>
          </div>
...
import { getWaterFeeList, createWaterFee, updateWaterFee, deleteWaterFee } from '@/api/waterFee'
...
const loadData = async () => {
  try {
    ...
    const response = await getWaterFeeList(params)
...
const handleEdit = (row) => {
  isEdit.value = true
  dialogTitle.value = '编辑水费记录'
  form.value = {
    id: row.id,
    paymentDate: row.paymentDate,
    amount: row.amount,
    remark: row.remark,
    images: [] // 编辑时清空图片，让用户重新上传
  }
  dialogVisible.value = true
}
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        if (isEdit.value) {
          await updateWaterFee(form.value.id, formData)
          ElMessage.success('更新成功')
        } else {
          await createWaterFee(formData)
